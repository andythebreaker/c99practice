...
Schedule every_one_ok(Heap *heap)
{
    ...
    for(int i = 0; i < 5; i++)
    {
        ...
        {
            Node *tmp=heap->Head;
            stuff2return.D[i].className[j]=anyway;
#ifdef LIMITER
            int limiter=0;
...
{
    int ch = countHeap(heap);
    DEBUGint(ch);//TODO:remove
    VD *tmp=(VD*)malloc(sizeof(VD)*ch);
    Node *ptr=NULL;
    ...
    while(ptr!=NULL)
    {
        LOOP_NUM(i,5){DEBUGlf(tmp[j].distance[i].d)}
...
kmGroup *Initkm(Heap *heap)
{
    ...
    LOOP_NUM(i,GROUPNUM){//TODO remove
    DEBUGint(r[i])
    }
    Node *ptr=heap->Head;
    int i=0;
    ...
    while(ptr!=NULL)
    {
        if(i==*rPTR)
        {
            DEBUGint(i);//TODO remove
            tmpPTR->center=ToVec(((MEMBER)(ptr->data))->schedule);
            initHeap(&(tmpPTR->table));
            rPTR++;
            tmpPTR++;
        }
        i++;
